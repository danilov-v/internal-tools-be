...
const validateCreatePersonnelRequest = ({ body }) => {
...
const validateGetPersonnelRequest = req => {
    const unitId = req.query.unitId;

    ow(unitId, 'unitId', ow.any(ow.string.numeric, ow.number.integer));
};

const validateGetPersonnelByIdRequest = req => {
    const personnelId = req.params.personnelId;

    ow(personnelId, 'personnelId', ow.any(ow.string.numeric, ow.number.integer));
};
